...
describe("showcase deck coverage", () => {
  ...
  const FEATURES: [string, (c: ChartConfig) => boolean][] = [
    ...
    [
      "scatter background bands",
      (c) => (c.kind === "scatter" || c.kind === "bubble") && !!c.decorations?.bands?.length,
    ],
    ...
    [
      "waterfall gap-to-target",
      (c) => c.kind === "waterfall" && c.data.series.some((s) => /^target$/i.test(s.name.trim())),
    ],
    ...
    [
      "diverging heatmap (negative values)",
      (c) => c.kind === "heatmap" && c.data.series.some((s) => s.values.some((v) => (v ?? 0) < 0)),
    ],
    ...
    [
      ...
      (c) => c.kind === "area" && c.data.series.some((s) => s.values.some((v) => (v ?? 0) < 0)),
    ],
    ...
    [
      "scatter continuous color row",
      (c) =>
        (c.kind === "scatter" || c.kind === "bubble") && c.data.series.some((s) => /^colou?r$/i.test(s.name.trim())),
    ],
    ...
    [
      "stacked100 with negatives",
      (c) => c.kind === "stacked100" && c.data.series.some((s) => s.values.some((v) => (v ?? 0) < 0)),
    ],
